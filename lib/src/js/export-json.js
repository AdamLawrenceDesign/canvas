...
function ExportJson(type, website, userId, userName)
{
	this.type = type;
...
{	
	return '?%userId=' + this.userId + '?%userName=' + this.userName + '?end'; 
};
...
ExportJson.prototype.updateWebJsons = function(obj, type)
{
	var _this;
	_this = this;
	...
	$.each(obj, function()
	{
		// console.log('This is missing Weblayout id: ', obj);
...
ExportJson.prototype.createNew = function()
{
	...
	$('header, aside').on('click', '.export', function()
	{
		...
		_this.serverCall('http://192.168.0.216/AdvAPI/api/WLValues', 'POST', newWebLayoutObj, function(WLValues)
		{
			...
			_this.pushWebJsons(webJsons, 'POST');
...
ExportJson.prototype.save = function()
{
	...
	$('header, aside').on('click', '.export', function()
	{
		...
		_this.serverCall('http://192.168.0.216/AdvAPI/api/WLValues/' + layoutData.id, 'PUT', newWebLayoutObj, function(WLValues)
		{
			...
			_this.updateWebJsons(webJsons, 'PUT');
		});	
